    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(),
          const SizedBox(height: 8),
          const Text('Inventory Notes', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          const SizedBox(height: 4),
          const Text('Checks live overstocked items from the Inventory component — separate from the model, for reference only.', style: TextStyle(fontSize: 11, color: AppColors.textMuted)),
  ...
  Widget _buildInventoryNote() {
    ...
    if (inventoryMatches == null || inventoryMatches!.isEmpty) {
      return Container(
        ...
        child: const Text('No overstocked items currently flagged for this segment.', style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
      );
    }
    return Container(
      padding: const EdgeInsets.all(10),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Text('Recommendation', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
              const Spacer(),
          ...
          const Divider(),
          const SizedBox(height: 8),
          const Text('Timing', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
          SwitchListTile(
      ...
      children: [
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('RECOMMENDED PROMOTION', style: TextStyle(color: AppColors.labelGold, fontSize: 12, letterSpacing: 1)),
              const SizedBox(height: 8),
              Text(top['offer_type'], style: const TextStyle(color: AppColors.goldAccent, fontSize: 26, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text('Predicted redemption probability: ${(top['probability'] * 100).toStringAsFixed(1)}%', style: const TextStyle(color: AppColors.textPrimary)),
            ],
          ),
        ),
        const SizedBox(height: 16),
        const Text('All Offer Scores', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
        const SizedBox(height: 8),
            ...
            child: Column(
              ...
              children: [
                Text('${offer['offer_type']}${isTop ? ' *' : ''}',
                    style: TextStyle(fontWeight: isTop ? FontWeight.bold : FontWeight.normal, color: AppColors.textPrimary, fontSize: 13)),
                const SizedBox(height: 4),
                ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    ...
                    color: isTop ? AppColors.goldAccent : AppColors.slateAccent,
                  ),
                ),
